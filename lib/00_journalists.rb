...
def nb_handle
  puts "Il y a #{journalists.length} dans cette array"
end

#Quel est le handle le plus court de cette liste ?
def 
  puts "Le handle le plus court de cette liste est: #{journalists.sort_by {|s| s.length}[0]}" #puts journalists.min {|a,b| a.length <=> b.length}
end
...
def capital_handle
  puts "Le nombre de handle qui commencent par une majuscule : #{journalists.count {|x| x[1].match?( /[[:upper:]]/) }}"
end

#Trie la liste de handle par ordre alphabétique.
def alphabetical_array
...
def 
  puts "Voici la liste triée du handle le plus court au plus long : #{journalists.sort_by(&:length)} " #{journalists.sort_by {|x| x.length}}
end

#Quelle est la position dans l'array de la personne @epenser ?
def
  puts "Voici la position dans l'array de la personne @epenser est : #{journalists.index {|x| x == "@epenser"}} "
end
...
def 
  puts " répartition des handle par taille de ces derniers : #{journalists.max {|a,b| a.length <==> b.length}}"
end

def repartition(journalists)
  number_character=0
  while number_character <=12
    puts "Le nombre de handle avec #{number_character} caractères est #{journalists.count {|x| x.length == number_character}}"
    number_character += 1    
  end
